...
async function removeBlockchainColumn() {
  const client = await pool.connect();

  try {
    console.log('Connected to database. Checking if blockchain column exists...');
    ...
    if (checkResult.rows.length > 0) {
      console.log('The blockchain column exists in wallet_addresses table. Removing it now...');
      
      // Remove the blockchain column
      ...
      console.log('Successfully removed blockchain column from wallet_addresses table.');
    } else {
      console.log('The blockchain column does not exist in wallet_addresses table.');
    }
