...
async function addLabelColumn() {
  ...
  try {
    console.log('Connected to database. Checking if label column exists...');
    ...
    if (checkResult.rows.length === 0) {
      console.log('The label column does not exist in wallet_addresses table. Adding it now...');
      ...
      console.log('Successfully added label column to wallet_addresses table.');
    } else {
      console.log('The label column already exists in wallet_addresses table.');
    }
